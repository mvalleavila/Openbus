...
class TcpProxyConnector implements TcpServerHandler {
	...
	public ByteBuffer readFromClient() throws IOException {
		ByteBuffer byteBuffer = serverBuffer.getByteBufferWriteFrom(clientChannel, config.isOnlyActiveChecks());
		...
		try {
			if (key.channel() == clientChannel) {
				if (key.isValid() && key.isReadable()) {
					ByteBuffer byteBuffer = readFromClient();
					if (byteBuffer != null) {
						IntegrationQueue.getInstance().insert(byteBuffer);
						if (config.isOnlyActiveChecks())
							destroy();
					}
				}
			...
			// LOGGER.log(Level.INFO, "Channel was closed by client or server.",
			// exception);
		} catch (final IOException exception) {
			destroy();
